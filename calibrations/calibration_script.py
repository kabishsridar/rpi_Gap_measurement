...
calib_data_path = "/home/kabish/python_projects/rpi_Gap_measurement/calibrated_data"
...
# h, w = image.shape[:2]
ret, mtx, dist, rvecs, tvecs = cv.calibrateCamera(obj_points_3D, img_points_2D, grayScale.shape[::-1], None, None) # caliberation part
print("calibrated")

print("dumping the data into one files using numpy ")
np.savez(
    f"{calib_data_path}/MultiMatrix_rpi",
    camMatrix=mtx,
    distCoef=dist,
    rVector=rvecs,
    tVector=tvecs,
...
data = np.load(f"{calib_data_path}/MultiMatrix_rpi.npz")
